...
export function RootDetectScreen() {
  ...
  if (!isAndroid) {
    return (
      <EmptyState
        title="Android Only"
        message="Root detection is only available on Android devices"
      />
    )
  }

  if (status === 'scanning') {
    return <LoadingState message="Scanning for root access..." />
  }

  if (status === 'error') {
    return (
      <ErrorState
        message={error || 'Failed to detect root status'}
        onRetry={refetch}
      />
    )
  }
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      <View style={styles.header}>
        <Text style={styles.title}>Root Detection</Text>
        <View
          style={[
            styles.badge,
            result.isRooted ? styles.badgeUnsafe : styles.badgeSafe,
          ]}
        >
          <Text style={styles.badgeText}>
            {result.isRooted ? 'ROOTED' : 'SECURE'}
          </Text>
        </View>
      </View>
      ...
      <View style={styles.card}>
        ...
        {result.detections.length === 0 ? (
          <Text style={styles.value}>No root indicators detected</Text>
        ) : (
