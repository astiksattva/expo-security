...
export function RecordingPreventDemoScreen() {
  const { state, error, enable, disable } = useRecordingPrevent()

  return (
    <View style={styles.container}>
      <Text style={styles.title}>Recording Prevent Demo</Text>

      <View style={styles.statusCard}>
        <Text style={styles.statusEmoji}>
          {state.isPrevented ? '🛡️' : '⚠️'}
        </Text>
        <Text
          style={[
            styles.statusText,
      ...
      {!state.available && (
        <View style={styles.warningBox}>
          <Text style={styles.warningText}>
            Recording prevention requires a Dev Build with native modules
          </Text>
        </View>
      )}
